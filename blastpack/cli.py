...
def cmd_radius(args):
    ...
    node = p["nodes"][i]
    print(f"{node['label']} ({node['id']}) reaches {len(reached)} principals:")
    for idx in reached:
        r = p["nodes"][idx]
...
def cmd_reachers(args):
    ...
    who = reachers_of(p, t)
    node = p["nodes"][t]
    print(f"{len(who)} principals can reach {node['label']} ({node['id']}):")
    for idx in who:
        r = p["nodes"][idx]
        print(f"  {r['label']}  [{r['type']}]")
    return 0
...
def cmd_top(args):
    ...
    for idx, size in metrics.top_by_radius(p, args.k):
        node = p["nodes"][idx]
...
def cmd_diff(args):
    ...
    for sid, ch in sorted(d["changed"].items(), key=lambda kv: label.get(kv[0], kv[0])):
        if args.focus_hv and not ((ch["added"] | ch["removed"]) & hv):
            continue
